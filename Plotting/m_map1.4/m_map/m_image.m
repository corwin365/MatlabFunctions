...
% Dec/2019  - added capability to merge with alphadata settings used in
%             inpyut (Thanks to W. Brown)
...
while k<length(varargin)
    switch lower(varargin{k}(1:3))
        case 'res'
            ...
            varargin([k k+1])=[];
            k=k-1;
        case 'alp' % user might want to set image alphadata
            alphadata_in = varargin{k+1};
            varargin([k k+1])=[];
            k=k-1;
     end
     k=k+1;
...
if nargout<=1
   if exist('alphadata_in', 'var')
      if length(alphadata_in) == 1
         alphadata = repmat(alphadata_in,size(GI,1),size(GI,2));
      else
         alphadata = alphadata_in;
      end
   else
      alphadata = ones(size(GI,1),size(GI,2),'logical');
   end
   if strcmp(class(C),'uint8') && length(size(C))==3
     if any(I)  % Points outside the map - set to zero visibility
        IJ=sub2ind(size(alphadata),I,J);
        alphadata(IJ)=0;
     end
     GI=image('XData',xlm,'YData',ylm,'CData',GI,'alphadata',alphadata,varargin{:},'tag','m_image');
   elseif strcmp(class(C),'double') && length(size(C))==2
     alphadata(isnan(GI)) = 0;
     GI=image('XData',xlm,'YData',ylm,'CData',GI,varargin{:},'tag','m_image',...
               'CDataMapping','scaled','alphadata',alphadata);
   end
end
